      ...
      if (err) {
        ...
        keys.map((ele) => {
          func.msCons.errorJson["message"] =
            err.errors[ele].path + " is " + err.errors[ele].kind;
...
const getUserListService = async (req) => {
  ...
  return new Promise(async (resolve, reject) => {
    let query = [
      {
        $match: {
          $and: [{ is_deleted: false }, { role: Number(req.params.user_role) }],
        },
      },
      {
        $lookup: {
    ...
    TempUserSchema.findOne({
      ...
      .then(async (result) => {
        ...
        if (result) {
          ...
          const query = {
            $and: [{ user_id: body.user_id }, { user_token: body.user_token }],
          };
          const update = {
...
module.exports = {
  ...
  linkedInCandidateDataService,
  linkedInCandidateEmail,
  getUserListService,
  tempUserRegisterService,
  getTempUserDataService,
  updateUserRegisterService,
};
